...
Тарифы:
  ...
  - Погружение: доступ 3 месяца, от 14 900 ₽. Включает "лимфопедию" (атлас УЗ-изображений
    ...
    есть одна онлайн-встреча, гайд (памятка) по всему курсу ее можно скачать.
...
образования, лимфоузлы, эластография, гайд (памятка) по всему курсу ее можно скачать и протоколы описания. В курс входит атлас
...
- Закрытый Telegram-канал УЗ-диагноста (алгоритмы, библиотека, комьюнити) —
  ...
  при заказе. Внутри библиотеки уже 1000+ материалов:
готовые алгоритмы по УЗД в гинекологии, уронефрологии, гепатобилиарной зоны и поверхностно расположенных органов на основе отечественных и зарубежных рекомендаций. 

=== СЕРТИФИКАТЫ ===
- Выдаются только по итогам КУРСОВ (не выдаются за интенсивы и прочие продукты).
- Сертификат внутреннего образца. Подойдет для вашего портфолио, для аккредитации его использовать нельзя.
...
О: Сертификат не дает право работать врачом ультразвуковой диагностики.
