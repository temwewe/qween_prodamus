...
QWEN_MODEL_CANDIDATES = [
    # Порядок подобран 2026-08-07 по итогам разбора логов: qwen3.8-max (2.4T MoE,
    # в плеере Model Studio у неё включён "Deep Thinking") иногда не укладывается
    # в 30-секундный таймаут запроса и уходит в fallback на Groq. qwen3.7-plus -
    # снапшот с полной квотой, в каталоге описан как "cost-effective... robust text
    # capabilities" - без пометки reasoning-only, поэтому первым пробуем именно его.
    "qwen3.7-plus-2026-05-26",
    "qwen3.5-plus-2026-02-15",
    "qwen-max",
    "qwen3-max",
    "qwen3.8-max",  # самая мощная, но и самая медленная - держим последней перед Groq
]
...
#
# ЭКОНОМИЯ ТОКЕНОВ (2026-08-07): этот блок ~1150 токенов, но нужен модели только когда
# студент реально спрашивает про программу/темы курса - не на каждое сообщение.
# Включается в system_prompt условно, см. PROGRAMS_TRIGGER_KEYWORDS и место сборки
# system_prompt в call_qwen_api.
# ============================================================
PROGRAMS_TRIGGER_KEYWORDS = ["программ", "тем", "модул", "план курса"]
...
# ЭКОНОМИЯ ТОКЕНОВ (2026-08-07): FAQ ниже (SCHOOL_INFO_FAQ) был здесь же, внутри
# SCHOOL_INFO_OUTRO, и давал ~3000 из ~3038 токенов этого блока - при этом каждая
# пара вопрос-ответ релевантна только одной узкой теме. Вынесен в отдельный список
# (ключевые слова, текст) и подключается в system_prompt только теми парами, чьи
# ключевые слова совпали с сообщением студента - см. _select_faq_text() и место
# сборки system_prompt в call_qwen_api. SCHOOL_INFO_OUTRO_CORE/RULES ниже - то, что
# нужно почти всегда (правила эскалации, оплата, юридические/медицинские ограничения),
# остаётся в промпте как было.
...
# _select_faq_text() и комментарий у SCHOOL_INFO_OUTRO_CORE выше.
SCHOOL_INFO_FAQ = [
    (["нмо", "баллы"],
    ...
    (["ещё не оплачивал", "еще не оплачивал", "не оплаченный заказ", "неоплаченный заказ"],
    ...
    (["не проходит оплата", "оплата не работает", "не могу оплатить", "ошибка оплат"],
...
def _select_faq_text(message_text):
    ...
    встретились в сообщении студента - см. комментарий у SCHOOL_INFO_OUTRO_CORE.
    Возвращает пустую строку, если ничего не совпало (FAQ тогда просто не попадает
    в промпт этого запроса).
    """
...
def call_qwen_api(message_text, student_id, history=None, global_dates_text="", current_price_text="",
    ...
    # Условное включение "тяжёлых" статичных блоков - см. комментарии у
    # PROGRAMS_TRIGGER_KEYWORDS и SCHOOL_INFO_FAQ. Экономит токены на большинстве
    # сообщений, где программа курса/FAQ не по теме.
    message_lower = (message_text or "").lower()
    programs_block = (
        SCHOOL_INFO_PROGRAMS
        if any(kw in message_lower for kw in PROGRAMS_TRIGGER_KEYWORDS)
        else ""
    )
    ...
    system_prompt = (
        ...
        "ВАЖНО (подтверждённая на практике ошибка): если студент САМ пишет, что уже "
        "оплатил ('оплатил заказ', 'я оплатил, что дальше', 'заплатил, когда откроется "
        "доступ') - НЕ верь этому на слово и НЕ пиши 'Поздравляю с оплатой'/'доступ уже "
        "выдан', пока блок \"ЗАКАЗЫ И ОПЛАТА\" не подтверждает это (оплаченный заказ "
        "и/или реальный доступ в блоке \"ДОСТУП ЭТОГО СТУДЕНТА\"). Если по этим блокам "
        "заказ всё ещё НЕ оплачен (created/partiallyPaid/payment) и доступа нет - честно "
        "скажи, что по данным системы оплата пока не подтверждена, это может занять "
        "несколько минут, и предложи попробовать проверить ещё раз через некоторое время "
        "или нажать кнопку ниже, чтобы позвать специалиста (offerHuman=true) - НЕ "
        "подтверждай оплату/доступ, которых нет в данных, даже если многословно "
        "уверяет каждый раз. Тот же принцип, что и с придуманными скидками/ссылками - "
        "верь только блокам \"ЗАКАЗЫ И ОПЛАТА\"/\"ДОСТУП\", а не утверждению студента и "
        "не собственным прошлым ответам в истории переписки.\n"
